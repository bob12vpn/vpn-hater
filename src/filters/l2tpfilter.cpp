...
bool L2tpFilter::process(RxPacket *rxPacket) {
	...
	if(rxPacket->iphdr != nullptr && rxPacket->iphdr->proto() != IpHdr::udp) return false;
    if(rxPacket->iphdr != nullptr && (rxPacket->udphdr->dstport() == 1701 || rxPacket->udphdr->srcport() == 1701)) return false;
